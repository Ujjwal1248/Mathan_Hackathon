...
export const useDisasterStore = create<DisasterStore>((set, get) => ({
  ...
  fetchInitialData: async () => {
    ...
    try {
      ...
      if (disastersError) {
        console.error('Error fetching disasters:', disastersError);
      }
      if (resourcesError) {
      ...
      // Use fetched data if available, otherwise keep initial mock data
      set({
        disasters: disasters && disasters.length > 0 ? disasters : initialDisasters,
        resources: resources && resources.length > 0 ? resources : initialResources,
        teams: teams && teams.length > 0 ? teams : initialTeams,
        reports: reports || [],
        alerts: alerts && alerts.length > 0 ? alerts : initialAlerts,
        loading: false
      });
    } catch (error) {
      ...
      set({ 
        error: 'Failed to fetch data from server, using mock data', 
        loading: false,
        disasters: initialDisasters,
        resources: initialResources,
        teams: initialTeams,
